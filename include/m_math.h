...
static unsigned int m_next_power_of_two(unsigned int x)
{
   if (x == 0)
      return 0;
   x--;
...
M_API void m_quat_identity(float4 *dest);
M_API void m_quat_normalize(float4 *dest, const float4 *src);
M_API void m_quat_rotation_axis(float4 *dest, const float3 *axis, float angle);
M_API void m_quat_rotation_euler(float4 *dest, const float3 *euler);
M_API void m_quat_mul(float4 *dest, const float4 *A, const float4 *B);
M_API void m_quat_slerp(float4 *dest, const float4 *A, const float4 *B, float mu);
M_API void m_quat_extract_axis_angle(float3 *axis, float *angle, const float4 *quat);
...
M_API void m_mat4_identity(float *dest);
M_API void m_mat4_perspective(float *dest, float fov, float ratio, float znear, float zfar);
M_API void m_mat4_ortho(float *dest, float left, float right, float bottom, float top, float znear, float zfar);
M_API void m_mat4_lookat(float *dest, const float3 *pos, const float3 *dir, const float3 *up);
M_API void m_mat4_translation(float *dest, const float3 *translation);
M_API void m_mat4_rotation_axis(float *dest, const float3 *axis, float angle);
M_API void m_mat4_rotation_euler(float *dest, const float3 *euler);
M_API void m_mat4_scale(float *dest, const float3 *scale);
M_API void m_mat4_mul(float *dest, const float *A, const float *B);
M_API void m_mat4_transpose(float *dest, const float *src);
M_API void m_mat4_inverse_transpose(float *dest, const float *src);
M_API void m_mat4_inverse(float *dest, const float *src);
M_API void m_mat4_extract_euler(float3 *dest, const float *matrix);
M_API void m_mat4_rotate3(float3 *dest, const float *matrix, const float3 *src);
M_API void m_mat4_inverse_rotate3(float3 *dest, const float *matrix, const float3 *src);
M_API void m_mat4_transform3(float3 *dest, const float *matrix, const float3 *src);
M_API void m_mat4_transform4(float4 *dest, const float *matrix, const float4 *src);
...
M_API int   m_3d_ray_sphere_intersection_in_out(float3 *ray_origin, float3 *ray_direction, float3 *sphere_origin, float sphere_radius2, float *in, float *out);
